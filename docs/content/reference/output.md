...
describes what users get. Pick a format with `-o`, or let marinemeteo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
marinemeteo <command> -o table   # aligned columns for reading
marinemeteo <command> -o jsonl   # one JSON object per line, for piping
marinemeteo <command> -o json    # a single JSON array
marinemeteo <command> -o csv     # spreadsheet friendly
marinemeteo <command> -o tsv     # tab-separated
marinemeteo <command> -o url     # just the URL column
marinemeteo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
marinemeteo <command> --fields id,title,url
```
...
```bash
marinemeteo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
marinemeteo <command>            # a table, because this is a terminal
marinemeteo <command> | wc -l    # JSONL, because this is a pipe
```
