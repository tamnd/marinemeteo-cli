// Package cli assembles the marinemeteo command tree from the marinemeteo
...
// NewApp assembles the kit application from the marinemeteo domain. The
...
//
// To add a command, declare it in marinemeteo/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := marinemeteo.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(marinemeteo.Domain{}).Register(app)
